...
const buildSystemPrompt = (config: DatasetConfig): string => {
  ...
  return `You are a Memory Assistant for the "${config.name}" demo. You help users explore and understand the memories stored in Redis Agent Memory.
...
- **Session memory**: Live session context including transcript events (messages)
...
- **Always state the search scope** in your response. Examples:
  ...
  - "From the **current session** and **all long-term memories**: ..."
