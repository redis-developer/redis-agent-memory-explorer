...
Eight LangChain `StructuredTool` instances wrapping `AgentMemory` methods. Each tool reads `namespace` and `userId` from `appState` (same as existing handlers).
...
const getComputedSummariesTool = new DynamicStructuredTool({
  ...
  func: async ({ viewName }) => {
    ...
    const targetViews = viewName
      ? views.filter((v) => v.name === viewName)
      : views;
...
- All memory tools respect the dataset's `namespace` and `userId` via `getAppState()` -- no cross-dataset data leakage.
